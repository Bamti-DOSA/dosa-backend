...
public class ModelObjectService {
    ...
    public List<ModelObjectResponse> getAllModels() {
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .description("고성능 BLDC 모터와 정밀 ESC 제어를 통해 안정적인 비행이 가능한 쿼드콥터 드론입니다. 탄소 섬유 프레임을 적용하여 경량화와 강성을 동시에 확보하였으며, 짐벌 시스템을 통한 흔들림 없는 영상 촬영 및 자율 비행 알고리즘 테스트에 최적화된 다목적 플랫폼입니다.")
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .description("고탄소강 재질의 여러 겹의 판을 겹쳐 만든 서스펜션 장치로, 대형 트럭이나 버스 등 고하중 차량의 충격 흡수에 탁월합니다. 구조가 간단하여 내구성이 뛰어나며, 자체적인 마찰 감쇠력을 통해 별도의 댐퍼 없이도 진동을 제어할 수 있어 경제적이고 유지보수가 용이한 것이 특징입니다.")
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .description("밀링 머신이나 머시닝 센터에서 공작물을 강력하고 정밀하게 고정하기 위한 필수 치공구입니다. 유압 증력 장치나 정밀 나사 기구를 통해 높은 체결력을 발휘하며, 죠(Jaw)의 평행도와 직각도가 매우 정밀하게 가공되어 있어 고정밀 절삭 가공 시 공작물의 떨림이나 이탈을 방지합니다.")
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .name("Robot Gripper")
                .type("Robotics & Automation")
                .description("로봇 팔의 말단(End-Effector)에 장착되어 물체를 파지하거나 조작하는 역할을 수행합니다. 공압 실린더 또는 전동 모터 구동 방식을 사용하며, 물체의 형상과 재질에 따라 핑거(Finger)를 교체할 수 있습니다. 힘 제어 센서를 통해 파지력을 조절하여 연약한 물체도 손상 없이 이송할 수 있습니다.")
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .name("Suspension")
                .type("Automotive & Parts")
                .description("노면의 충격을 흡수하여 승차감을 개선하고 타이어의 접지력을 유지하는 차량의 핵심 장치입니다. 코일 스프링이 충격 에너지를 저장하고, 쇽업쇼버(Shock Absorber)가 이를 열에너지로 소산시켜 차체의 진동을 억제합니다. 더블 위시본, 맥퍼슨 스트럿 등 다양한 방식이 있으며 주행 안정성에 결정적인 역할을 합니다.")
        ...
        models.add(ModelObjectResponse.builder()
                ...
                .description("실린더를 V자 형태로 4개 배치하여 컴팩트한 크기와 높은 출력을 동시에 구현한 내연기관입니다. 직렬 엔진 대비 크랭크축 길이가 짧아 비틀림 강성이 우수하며, 독특한 배기음과 부드러운 회전 질감이 특징입니다. 고성능 모터사이클이나 스포츠카에 주로 사용되며, 복잡한 밸브 트레인 설계가 요구됩니다.")
